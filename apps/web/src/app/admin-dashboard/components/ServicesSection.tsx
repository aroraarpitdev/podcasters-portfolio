              ...
              <div key={index} className="bg-[#111111] border border-[#2A2A2A] rounded-lg p-4 flex flex-col gap-4 relative group">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[12px] text-on-surface font-bold uppercase">{item.cardHeading || "Untitled Service"}</span>
                  </div>
                  <button className="material-symbols-outlined text-[18px] text-[#F0EDE680] hover:text-error" onClick={() => removeArrayItem("services", "serviceCards", index)}>delete</button>
                </div>
                {isItemExpanded && (
