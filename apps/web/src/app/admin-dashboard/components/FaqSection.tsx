          ...
          <div className="grid grid-cols-1 gap-4">
            {faqContent.map((item: any, index: number) => (
              <div key={index} className="bg-[#111111] border border-[#2A2A2A] rounded-lg p-4 flex gap-4 relative group">
                ...
                <button className="material-symbols-outlined text-[18px] text-[#F0EDE680] hover:text-error absolute top-4 right-4" onClick={() => removeArrayItem("faq", "faqContent", index)}>delete</button>
              </div>
            ))}
          </div>
        </div>
