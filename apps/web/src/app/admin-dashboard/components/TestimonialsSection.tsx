...
export default function TestimonialsSection() {
  ...
  const [expandedVideoCards, setExpandedVideoCards] = useState<Record<number, boolean>>({});
  const [expandedTextCards, setExpandedTextCards] = useState<Record<number, boolean>>({});
          ...
          <div className="space-y-4 pt-6 border-t border-[#2A2A2A]/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              {metricsStatsBar.map((item: any, index: number) => {
                const isItemExpanded = expandedMetrics[index] ?? (index === 0);
                return (
                  <div key={index} className="bg-[#111111] border border-[#2A2A2A] rounded-lg p-4 flex flex-col gap-4 relative group">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <button onClick={() => setExpandedMetrics(prev => ({ ...prev, [index]: !isItemExpanded }))} className="text-[#F0EDE680] hover:text-primary transition-colors">
                        ...
                        <span className="text-[12px] text-on-surface font-bold uppercase">{item.dataTitle || "Untitled Metric"}</span>
                      </div>
                      {/* <button className="material-symbols-outlined text-[18px] text-[#F0EDE680] hover:text-error" onClick={() => removeArrayItem("testimonials", "metricsStatsBar", index)}>delete</button> */}
                    </div>
                    {isItemExpanded && (
          ...
          <div className="space-y-4 pt-6 border-t border-[#2A2A2A]/30">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {videoCards.map((item: any, index: number) => {
                const isItemExpanded = expandedVideoCards[index] ?? (index === 0);
                return (
                  <div key={index} className="bg-[#111111] border border-[#2A2A2A] rounded-lg p-4 flex flex-col gap-4 relative group">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <button onClick={() => setExpandedVideoCards(prev => ({ ...prev, [index]: !isItemExpanded }))} className="text-[#F0EDE680] hover:text-primary transition-colors">
                          <span className="material-symbols-outlined text-[20px] transition-transform duration-300" style={{ transform: isItemExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>expand_more</span>
                        </button>
                        <span className="text-[12px] text-on-surface font-bold uppercase">{item.videoAuthor || "Untitled Video"}</span>
                      </div>
                      <button className="material-symbols-outlined text-[18px] text-[#F0EDE680] hover:text-error" onClick={() => removeArrayItem("testimonials", "videoCards", index)}>delete</button>
                    </div>
                    {isItemExpanded && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                        ...
                        <div className="flex flex-col gap-1">
                          <label className="text-[10px] text-[#F0EDE680] opacity-40 uppercase font-bold">Card Direction</label>
                          <select
                            className="w-full bg-[#0A0A0A] border border-[#2A2A2A] text-on-background p-2 outline-none focus:border-primary transition-colors appearance-none cursor-pointer"
                            value={item.cardDirection || "LTR"}
                            onChange={(e) => updateArrayItem("testimonials", "videoCards", index, "cardDirection", e.target.value)}
                          >
          ...
          <div className="space-y-4 pt-6 border-t border-[#2A2A2A]/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {textCards.map((item: any, index: number) => {
                const isItemExpanded = expandedTextCards[index] ?? (index === 0);
                return (
                  <div key={index} className="bg-[#111111] border border-[#2A2A2A] rounded-lg p-4 flex flex-col gap-4 relative group">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <button onClick={() => setExpandedTextCards(prev => ({ ...prev, [index]: !isItemExpanded }))} className="text-[#F0EDE680] hover:text-primary transition-colors">
                          <span className="material-symbols-outlined text-[20px] transition-transform duration-300" style={{ transform: isItemExpanded ? 'rotate(180deg)' : 'rotate(0deg)' }}>expand_more</span>
                        </button>
                        <span className="text-[12px] text-on-surface font-bold uppercase">{item.author || "Untitled Text Card"}</span>
                      </div>
                      <button className="material-symbols-outlined text-[18px] text-[#F0EDE680] hover:text-error" onClick={() => removeArrayItem("testimonials", "textCards", index)}>delete</button>
                    </div>
                    {isItemExpanded && (
                      <div className="flex flex-col gap-4 mt-2">
                        <div className="flex flex-col gap-1">
